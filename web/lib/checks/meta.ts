...
export function checkMeta(html: string): CheckResult[] {
  ...
  $('script[type="application/ld+json"]').each((_i, el) => {
    ...
    } catch { /* invalid JSON already caught above */ }
  })
  if (foundTypes.length === 0) {
    ...
    if (geoMatches.length === 0) {
      results.push({ name: 'schema types', status: 'warn', detail: `@type encontrado (${foundTypes.join(', ')}) pero ninguno es tipo GEO relevante`, recommendation: `Usa al menos uno de: ${GEO_TYPES.slice(0, 6).join(', ')}.`, score: 3, maxScore: 8 })
    } else {
