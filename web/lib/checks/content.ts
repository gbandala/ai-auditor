...
export function checkContent(html: string): CheckResult[] {
  ...
  const missingAlt = images.filter((_i, el) => { const alt = $(el).attr('alt'); return alt === undefined || alt.trim() === '' }).length
  if (totalImgs === 0) {
    ...
    results.push({ name: 'alt text', status: 'warn', detail: `${missingAlt} de ${totalImgs} imágenes sin alt text`, recommendation: 'Agrega atributo alt descriptivo a todas las imágenes.', score: 3, maxScore: 7 })
  } else {
  ...
  if (foundLandmarks.length === 0) {
    ...
    results.push({ name: 'semantic html', status: 'warn', detail: `Landmarks presentes (${foundLandmarks.join(', ')}) pero sin main ni article`, recommendation: 'Agrega main alrededor del contenido principal y article en contenido citable.', score: 2, maxScore: 5 })
  } else {
